...
const NAV_ITEMS = [
  { label: 'Hub',             route: '/',           icon: HomeIcon },
  { label: 'Ticket Analyser', route: '/analyser',   icon: SearchIcon },
  { label: 'BU/PS Tickets',   route: '/bu-tickets', icon: TableIcon },
      ...
      <div className={`border-t border-slate-700 p-3 flex items-center gap-3 ${collapsed ? 'justify-center' : ''}`}>
        ...
        {!collapsed && (
          <div className="flex-1 min-w-0">
            ...
            <button
              onClick={signOut}
              className="text-slate-500 hover:text-red-400 text-xs mt-0.5 transition"
            >
              Sign out
            </button>
          </div>
