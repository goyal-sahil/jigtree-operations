...
export default function BUTicketsToolbar({ lastSyncedAt, isAdmin, totalCount }: Props) {
  ...
  async function handleSync() {
    ...
    try {
      ...
      const body = await res.json() as {
        synced: number; failed: number; total: number; duration: number; firstError?: string
      }
      if (body.synced === 0) {
        ...
        setSyncStatus(`Synced ${body.synced}/${body.total} tickets in ${(body.duration / 1000).toFixed(1)}s${suffix} — fetching posts & queuing analysis`)
