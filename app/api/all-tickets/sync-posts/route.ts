...
export async function POST(request: NextRequest) {
  ...
  // Non-BU/PS tickets that still need posts fetched
  const tickets = await prisma.ticket.findMany({
    where:   { isBuPs: false, kayakoUrl: settings.kayakoUrl, postsStatus: { not: 'done' } },
  ...
  for (const ticket of tickets) {
    ...
    try {
      ...
      console.error(`[all-tickets-sync-posts] Failed ticket ${ticket.kayakoTicketId}:`, err instanceof Error ? err.message : err)
