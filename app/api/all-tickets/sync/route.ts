...
const ALL_TICKETS_VIEW_ID = 242
const BU_PS_VIEW_ID       = 64
const BATCH_SIZE          = 5
...
export async function POST(request: NextRequest) {
  ...
  // ── 3. Fetch all case stubs from view #242 ────────────────────────────────
  let viewCases: KayakoCase[]
  try {
    viewCases = await client.getViewCases(ALL_TICKETS_VIEW_ID)
  } catch (err: unknown) {
    return NextResponse.json(
      { error: err instanceof Error ? err.message : 'Failed to fetch view #242 cases' },
      { status: 502 },
    )
  }

  console.log(`[all-tickets-sync] View #242 returned ${viewCases.length} case stubs`)

  // ── 4. Load existing closed ticket IDs (skip re-syncing them) ────────────
  const existingDb = await prisma.ticket.findMany({
    where:  { kayakoUrl: settings.kayakoUrl },
    select: { kayakoTicketId: true, status: true },
  })
  const existingByKayakoId = new Map(existingDb.map(t => [t.kayakoTicketId, t]))

  // ── 5. Upsert each case into tickets table ────────────────────────────────
  let synced  = 0
  let skipped = 0
  let failed  = 0
  ...
  for (let i = 0; i < viewCases.length; i += BATCH_SIZE) {
    if (signal.aborted) {
      console.log('[all-tickets-sync] Client disconnected — stopping')
      break
    }
    const batch = viewCases.slice(i, i + BATCH_SIZE)
    console.log(`[all-tickets-sync] Batch ${Math.floor(i / BATCH_SIZE) + 1}/${Math.ceil(viewCases.length / BATCH_SIZE)} — cases ${i + 1}–${Math.min(i + BATCH_SIZE, viewCases.length)}`)

    await Promise.allSettled(
      batch.map(async (stub) => {
        // Skip re-syncing tickets already in DB with a closed status
        const existing = existingByKayakoId.get(stub.id)
        if (existing && existing.status?.toLowerCase().includes('closed')) {
          skipped++
          return
        }
        ...
        try {
          ...
          console.error(`[all-tickets-sync] Failed case ${stub.id}:`, msg)
  ...
  // ── 6. Reset isBuPs = false for ALL tickets at this kayakoUrl ────────────
  await prisma.ticket.updateMany({
    where: { kayakoUrl: settings.kayakoUrl },
    data:  { isBuPs: false },
  })
  console.log('[all-tickets-sync] Reset all isBuPs = false')

  // ── 7. Fetch view #64 case stubs (IDs only) and set isBuPs = true ─────────
  try {
    const buPsCases = await client.getViewCases(BU_PS_VIEW_ID)
    const buPsIds   = buPsCases.map(c => c.id)
    if (buPsIds.length > 0) {
      await prisma.ticket.updateMany({
        where: { kayakoTicketId: { in: buPsIds }, kayakoUrl: settings.kayakoUrl },
        data:  { isBuPs: true },
      })
      console.log(`[all-tickets-sync] Set isBuPs = true for ${buPsIds.length} BU/PS tickets`)
    }
  } catch (err) {
    // Non-fatal: isBuPs flags will be stale until next BU/PS sync, but All Tickets data is valid
    console.warn('[all-tickets-sync] Could not fetch view #64 for isBuPs update:', err instanceof Error ? err.message : err)
  }

  const duration = Date.now() - started
  return NextResponse.json({ ok: true, synced, skipped, failed, total: viewCases.length, duration, firstError })
}
